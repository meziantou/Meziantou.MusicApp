...
public class RestApiController(MusicLibraryService library, TranscodingService transcoding, ImageResizingService imageResizing, LastFmService lastFm, ILogger<RestApiController> logger) : ControllerBase
{
    ...
    /// <summary>Compute replay gain for a track</summary>
    /// <remarks>
    /// Analyzes the audio file and computes replay gain values (track gain and peak).
    /// The computed values are saved to the file's metadata tags and the library cache.
    /// This is useful for fixing tracks that are missing replay gain information.
    /// </remarks>
    [HttpPost("songs/compute-replay-gain")]
    [ProducesResponseType<ComputeReplayGainResponse>(StatusCodes.Status200OK)]
    [ProducesResponseType<ErrorResponse>(StatusCodes.Status404NotFound)]
    [ProducesResponseType<ErrorResponse>(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<ComputeReplayGainResponse>> ComputeReplayGain([FromBody] ComputeReplayGainRequest request)
    {
        if (string.IsNullOrEmpty(request.Id))
        {
            return BadRequest(new ErrorResponse { Error = "Song ID is required" });
        ...
        try
        {
            var result = await library.ComputeSongReplayGainAsync(request.Id);

            if (result is null)
            {
                return Ok(new ComputeReplayGainResponse
                {
                    Success = false,
                    Id = request.Id,
                    Title = song.Title,
                    Message = "Failed to compute replay gain",
                });
            }
            ...
            return Ok(new ComputeReplayGainResponse
            {
                ...
                Message = $"Replay gain computed successfully: {result.TrackGain:F2} dB",
            });
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Error computing replay gain for song {SongId}", request.Id);
            return StatusCode(500, new ErrorResponse { Error = "Failed to compute replay gain" });
        }
    }
}
